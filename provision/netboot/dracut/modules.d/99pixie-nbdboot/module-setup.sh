#!/bin/bash
# dracut module: pixie-nbdboot
#
# Install-time hook. Called by ``dracut -f`` (or dracut's automatic
# runs after a kernel update) when 99pixie-nbdboot is in the module
...
install() {
    ...
    inst_hook cmdline 10 "$moddir/pixie-nbdboot-cmdline.sh"
    inst_hook initqueue/online 20 "$moddir/pixie-nbdboot-online.sh"
    inst_hook mount 90 "$moddir/pixie-nbdboot-mount.sh"
